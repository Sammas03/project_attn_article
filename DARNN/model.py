...
class DARNN(AbsModel):
    ...
    def __init__(self, config):
        super().__init__()
        encoder_input_size = config['sup_count'] # 辅助序列的个数
        self.lr = config['lr']
        self.darnn = AutoEncForecast(config)

    def forward(self, x:torch.Tensor):
        main_seq = x[:,0,:]
        sup_seq = x[:,1:,:].permute(0,2,1) # 为了适应源代码中的变形
        out = self.darnn(sup_seq,main_seq)
        return out
...
class AttnEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.input_size = config['sup_count'] #lstm 输入的数量和辅助序列个数相同
...
class AttnDecoder(nn.Module):
    ...
    def forward(self, input_encoded: torch.Tensor, y_history: torch.Tensor):
        ...
        for t in range(self.seq_len):
            ...
            y_tilde = self.fc(torch.cat((context, y_history[:,t].unsqueeze(1)),
...
class AutoEncForecast(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = AttnEncoder(config)
