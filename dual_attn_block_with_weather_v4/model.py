...
class MainModel(AbsModel):
    ...
    def forward(self, x):
        ...
        # return output
        # 相比较源模型 ，做出了变形将原始数据分离
        main_seq = x[:, 0, :].unsqueeze(1).permute(0, 2, 1)
...
class TemproalDecoder(nn.Module):
    ...
    def forward(self, input_encoded: torch.Tensor, his: torch.Tensor, weather_encoded: torch.Tensor):
        ...
        for t in range(self.prediction_horizon):  # 这里是 seq2seq 循环控制需要预测多少个值
            ...
            d_t_cat = torch.cat((d_t, weather_encoded.unsqueeze(0)), dim=2)
            out = self.fc_out(d_t_cat.permute(1, 0, 2))  # predicting value at t=self.seq_length+1
